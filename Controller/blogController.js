...
const editBlog = async (req, res) => {
  try {
    ...
    // Update Blog
    const blogData = await blogModel.findByIdAndUpdate(_id, req.body, {
      new: true,
    });
...
const getBlogById = async (req, res) => {
  ...
  try {
    ...
    // Find blog where both blog id and user id match
    const blogData = await blogModel.findOne({ _id: bId, user_id });

    if (!blogData) {
      // Not found
      return res.status(404).json({ msg: "You Are Not Authorize - Blog not found." });
    }
...
export { createBlog, editBlog, deleteBlog, getAllBlog, getBlogById };
